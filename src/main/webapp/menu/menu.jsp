...
<t:formvalid formid="formobj" layout="div" dialog="true" refresh="true" action="menuController.do?saveFunction">
	...
	<div class="form" id="pfun"><label class="Validform_label"> 父菜单: </label> <input id="cc" <c:if test="${function.menu.functionLevel eq 0}">
					value="${function.menu.id}"</c:if>
		<c:if test="${function.menu.functionLevel > 0}">
					value="${function.menu.functionName}"</c:if>> <input id="functionId" name="menu.id" style="display: none;"
		value="${function.menu.id}"></div>
